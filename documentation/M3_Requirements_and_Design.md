...
It allows people to keep a history of all the places they have traveled to and add notes to each place, which act as travel advisories for others. Users will also receive suggestions on some interesting travel locations based on all user history (recommendation system algorithm). Since the app also allows a chatting feature, people can contact other users who have traveled there for advice easily.	
        ...
        2. **Delete Posts**
	     ...
	     4. App removes the delete post as a pin on the map 
            - **Failure scenario(s)**:
                ...
                - 5a. Post could not be deleted
                    - 5a1. A toast will appear telling the user that the post was unable to delete at this time along with the reason why
...
1. **User**
    ...
    - **Interfaces**: 
        1. `public static String register (String username, String password) throws UserRegistrationFailedException`
            - **Purpose**: register a user by storing username and hased-password pair into the database. On success, return the registered username, otherwise throws UserRegistrationFailedException.

        2. `public static AuthResponse login (String username, String password) throws AuthException`
            - **Purpose**: authenticate user login. On success returns AuthResponse object, containing token and username of the authenticated user. Otherwise throws AuthException


2. **Post**
    - **Purpose**: Manages posts data and allows posts retrieval based on location range or user's map view.
    - **Interfaces**: 
        1. `public static String uploadPost (String username, String description, Location location, List<Image> images, bool isPrivate)`
        ...
        2. `public static String uploadImages (String username, List<Image> images, bool private)`
...
3. **Chat**
    ...
    - **Interfaces**: 
        1. `public static void sendMessage (String senderUsername, String receiverUsername String message)`
...
4. **Recommendation System**
    ...
    - **Interfaces**: 
        1. `public static List<Post> getPostRecommendation(String username, Location userLocation)`
            - **Purpose**: Recommend posts based on user's travel history and user's current location.


### **4.2. Databases**
1. **UserDB**
    - **Purpose**: Stores user's credentials (hashed passwords), and username.
2. **Posts**
